...
fun PriceGraph(
    ...
    Canvas(modifier = modifier) {
        ...
        val linePath = if (minPrice != maxPrice) {
            Path().apply {
            ...
            Path().apply {
                moveTo(0f, height / 2)
                lineTo(prices.lastIndex * spacePerPoint, height / 2)
            }
        }
        ...
        drawLinePath(
            linePath = if (isLineAnimated) animatedLinePath.value else linePath,
            lineColor = graphLineColor
        )

        drawFillPath(
            fillPath = linePath.apply {
                lineTo((prices.size * spacePerPoint) + 1000, size.height)
